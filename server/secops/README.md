...
### Rule Exclusions Management Tools

- **`create_rule_exclusion(display_name, refinement_type, query, project_id=None, customer_id=None, region=None)`**
    - Create a new rule exclusion to filter out false positives or exclude specific events from triggering detections. Rule exclusions use UDM query syntax to define which events should be excluded.

- **`get_rule_exclusion(exclusion_id, project_id=None, customer_id=None, region=None)`**
    - Retrieve detailed information about a specific rule exclusion by ID, including display name, query, refinement type, and deployment status.

- **`list_rule_exclusions(page_size=None, page_token=None, project_id=None, customer_id=None, region=None)`**
    - List all rule exclusions configured in Chronicle with pagination support for reviewing and managing exclusions.
...
- **`patch_rule_exclusion(exclusion_id, display_name=None, query=None, update_mask=None, project_id=None, customer_id=None, region=None)`**
    - Update an existing rule exclusion's properties including display name and query. Supports partial updates via update_mask.
...
- **`update_rule_exclusion_deployment(exclusion_id, enabled, archived, detection_exclusion_application, project_id=None, customer_id=None, region=None)`**
    - Manage deployment settings for a rule exclusion including enable/disable status, archive status, and which rules or rule sets the exclusion applies to.

- **`compute_rule_exclusion_activity(exclusion_id, start_time, end_time, project_id=None, customer_id=None, region=None)`**
    - Calculate activity statistics showing how many events were excluded during a specified time period. Helps measure exclusion effectiveness and impact.
...
15. **Rule Exclusions Management**: Create, manage, and monitor rule exclusions to filter false positives and tune detection rules
16. **Watchlist Management**: Create, update, delete, and list watchlists for entity risk scoring
17. **Investigation Management**: Manage cases and investigations, trigger investigations, and fetch associated investigations
18. **UDM Search & Export**: Direct UDM querying, field value autocomplete, and CSV export
...
- **Rule Exclusions Management**: Use `create_rule_exclusion`, `get_rule_exclusion`, `list_rule_exclusions`, `patch_rule_exclusion`, `update_rule_exclusion_deployment`, and `compute_rule_exclusion_activity` to manage false positive filtering and detection rule tuning
