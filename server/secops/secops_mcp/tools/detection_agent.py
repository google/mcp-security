...
async def generate_threat_detection_opportunity(
    ...
    **Security Note:**
    ...
    """
    try:
        raw_threat = threat or threat_text
        if not raw_threat or not raw_threat.strip():
            return {
                "error": "The 'threat' parameter is required and cannot be empty.",
...
async def generate_synthetic_events(
    threat_detection_opportunity: dict[str, Any] | str | None = None,
    threatDetectionOpportunity: dict[str, Any] | str | None = None,
    project_id: str | None = None,
    customer_id: str | None = None,
    region: str | None = None,
    timeout: int = 300,
    ...
    **Parameter Requirements:**
    - `threat_detection_opportunity`: MUST be the exact, unmodified Threat Detection Opportunity (TDO)
      object returned by `generate_threat_detection_opportunity` (or JSON string).
    - The TDO MUST include a populated `log_types` list (e.g., `["WINEVTLOG", "EDR"]`).
    ...
    **Workflow Integration:**
    ...
    Args:
        threat_detection_opportunity (Optional[Union[Dict[str, Any], str]]): The TDO object or JSON string.
        threatDetectionOpportunity (Optional[Union[Dict[str, Any], str]]): Alias for threat_detection_opportunity.
        project_id (Optional[str]): Google Cloud project ID. Defaults to environment configuration.
        customer_id (Optional[str]): Chronicle customer ID. Defaults to environment configuration.
        region (Optional[str]): Chronicle region (e.g., "us", "europe"). Defaults to environment configuration.
        timeout (int): Request timeout in seconds. Defaults to 300s (5 minutes).
    ...
    """
    try:
        tdo_input = threat_detection_opportunity or threatDetectionOpportunity
        if not tdo_input:
            return {
                "error": "The 'threat_detection_opportunity' parameter is required.",
                "synthetic_events": [],
            }

        if isinstance(tdo_input, str):
            try:
                tdo = json.loads(tdo_input)
            ...
            return {
                "error": "'threat_detection_opportunity' must be a dictionary or a JSON string.",
                "synthetic_events": [],
            }

        # Check for populated log_types
        raw_log_types = tdo.get("log_types") or tdo.get("logTypes")
        if (
            not raw_log_types
            or not isinstance(raw_log_types, list)
            or len(raw_log_types) == 0
        ):
            return {
                "error": "The TDO MUST include a populated 'log_types' list (e.g., ['WINEVTLOG']).",
                "synthetic_events": [],
            }

        # Normalize log_types: extract string names if provided as objects
        ...
        if not clean_log_types:
            return {
                "error": "The TDO MUST include a populated 'log_types' list with valid log type strings.",
                "synthetic_events": [],
            }
        tdo["log_types"] = clean_log_types

        # Normalize summary if passed as threat_description or description
        if "summary" not in tdo:
            desc = tdo.pop("threat_description", None) or tdo.pop("description", None)
        ...
        if hasattr(type(chronicle), "generate_synthetic_events"):
            return chronicle.generate_synthetic_events(threat_detection_opportunity=tdo)
...
async def evaluate_rule_coverage_long_running(
    ...
    **Instructions for Polling:**
    ...
    try:
        raw_events = (
            threat_detection_opportunity_events or threatDetectionOpportunityEvents
        )
        if not raw_events:
            return {
...
async def get_operation(
    ...
    try:
        if not name or not name.strip():
            return {"error": "The 'name' parameter is required."}
...
async def generate_rules(
    ...
    Args:
        threat_detection_opportunity: The TDO object or JSON string from generate_threat_detection_opportunity.
    ...
    try:
        ...
        if hasattr(type(chronicle), "generate_rules"):
            return chronicle.generate_rules(threat_detection_opportunity=tdo)
