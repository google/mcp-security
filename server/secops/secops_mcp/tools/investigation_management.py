...
async def get_cases(
    ...
    """Batch retrieve case details using the legacy batch API.

    Retrieves detailed information for multiple cases in a single request.
    Supports up to 1000 case IDs per request. Returns case details including
    priority, status, stage, and SOAR platform information.

    **Workflow Integration:**
    - Use after retrieving alerts to get associated case details
    - Essential for incident response workflows where AI helps analysts
      navigate cases
    - Can be used to check case status before creating new investigations
    - Helps correlate alerts with existing case management workflows

    **Use Cases:**
    - "Show me the case details for these alert IDs"
    - "Get details for all high-priority cases"
    - "What's the status of cases associated with these alerts?"
    - "Retrieve case information for incident response"

    Args:
        case_ids (List[str]): List of case IDs to retrieve (max 1000).
    ...
    Returns:
        Dict[str, Any]: Dictionary containing case details including display
            name, priority, status, stage, and SOAR platform info. Returns
            error message if retrieval fails.

    Next Steps (using MCP-enabled tools):
        - Analyze case priority and status to determine next actions
        - Use `list_investigations` or `fetch_associated_investigations` to
          find related investigations
        - Use `get_security_alerts` to find alerts associated with cases
        - For high-priority cases, use `trigger_investigation` to create
          new investigations
        - Use entity lookup tools on indicators found in case details
    """
    try:
        if not case_ids:
            return {
                "error": "case_ids parameter is required and cannot be empty"
            }

        if len(case_ids) > 1000:
            return {
                "error": (
                    "Too many case IDs provided. Maximum is 1000, "
                    f"received {len(case_ids)}"
                )
            }

        chronicle = get_chronicle_client(project_id, customer_id, region)
        print(f"Retrieving details for {len(case_ids)} case(s)...")

        cases_response = chronicle.get_cases(case_ids)

        if not cases_response or not hasattr(cases_response, "cases"):
            return {"message": "No cases found", "case_count": 0, "cases": []}

        print(f"Successfully retrieved {len(cases_response.cases)} case(s)")
