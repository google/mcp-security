...
async def get_threat_intel(
    ...
    **Workflow Integration:**
    - Use this tool anytime during an investigation to quickly get context or summaries
      on specific threats, vulnerabilities, or concepts encountered.
    - Can provide background information before diving deeper with other tools like
      `lookup_entity` or `search_security_events`.
    - Useful for understanding the potential impact or nature of a CVE found in a
      SOAR case or vulnerability scan.
    ...
    **Use Cases:**
    ...
    Next Steps:
        - Use the provided summary or explanation to inform further investigation steps.
        - If the query was about an indicator (IOC, CVE), consider using `lookup_entity`
          or `search_security_events` to see if it has been observed in your environment.
        - Add relevant findings as comments to SOAR cases (`secops-soar:post_case_comment`).
    """
    try:
