...
def register_tools(mcp: FastMCP):
  ...
  @mcp.tool()
  async def varonis_data_security_platform_update_alert(
      ...
      alert_guid: Annotated[
          str,
          Field(
              ...,
              description=(
                  "Specify alert GUID for the update. This action can run on multiple"
                  " alerts. Multiple alerts can be specified as a comma-separated"
                  " string."
              ),
          ),
      ],
      alert_status: Annotated[
          List[Any], Field(..., description="Specify the alert status to update to.")
      ],
      closing_reason: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Specify the closing reason for the alert. When the alert status is"
                  ' changed to "closed", closing reason must be specified.'
              ),
          ),
      ],
    ...
    try:
      instance_response = await bindings.http_client.get(
          Endpoints.LIST_INTEGRATION_INSTANCES.format(
              INTEGRATION_NAME="VaronisDataSecurityPlatform"
          )
      )
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              ...
              "ScriptName": (
                  "VaronisDataSecurityPlatform_Update Alert"
      ...
      try:
        ...
        print(
            "Error executing action VaronisDataSecurityPlatform_Update Alert for"
            f" VaronisDataSecurityPlatform: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
      print(
          f"Warning: No active integration instance found for"
          f" VaronisDataSecurityPlatform"
      )
      return {"Status": "Failed", "Message": "No active instance found."}

  @mcp.tool()
  async def varonis_data_security_platform_ping(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              "IntegrationInstance": instance_identifier,
              "ScriptName": (
                  "VaronisDataSecurityPlatform_Ping"
              ),  # Assuming same as actionName
              "ScriptParametersEntityFields": json.dumps(script_params),
          },
      )
      ...
      try:
        ...
        # Log error appropriately
        print(
            "Error executing action VaronisDataSecurityPlatform_Ping for"
            f" VaronisDataSecurityPlatform: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
