...
def register_tools(mcp: FastMCP):
  ...
  @mcp.tool()
  async def site24x7_generate_refresh_token(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
      authorization_code: Annotated[
          str, Field(..., description="Specify the authorization code.")
      ],
      target_entities: Annotated[
          List[TargetEntity],
          Field(
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              "IntegrationInstance": instance_identifier,
              "ScriptName": (
                  "Site24x7_Generate Refresh Token"
      ...
      try:
        ...
        # Log error appropriately
        print(
            f"Error executing action Site24x7_Generate Refresh Token for Site24x7: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
      print(f"Warning: No active integration instance found for Site24x7")
