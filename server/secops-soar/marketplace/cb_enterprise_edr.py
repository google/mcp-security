...
def register_tools(mcp: FastMCP):
  ...
  @mcp.tool()
  async def cb_enterprise_edr_enrich_hash(
    ...
    if instances:
      ...
      try:
        ...
        print(
            "Error executing action CBEnterpriseEDR_Enrich Hash for"
            f" CBEnterpriseEDR: {e}"
        )
  ...
  @mcp.tool()
  async def cb_enterprise_edr_process_search(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
      ...
      query: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Query to execute in process search. For example,"
                  " process_name:svchost.exe - to search based by process name,"
                  " process_hash:9520a99e77d6196d0d09833146424113 - to search based by"
                  " process hash."
              ),
          ),
      ],
      ...
      sort_by: Annotated[
          Optional[str],
          Field(default=None, description="Specify a parameter for sorting the data."),
      ],
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              "IntegrationInstance": instance_identifier,
              "ScriptName": (
                  "CBEnterpriseEDR_Process Search"
              ),  # Assuming same as actionName
              "ScriptParametersEntityFields": json.dumps(script_params),
          },
      )
      ...
      try:
        ...
        # Log error appropriately
        print(
            "Error executing action CBEnterpriseEDR_Process Search for"
            f" CBEnterpriseEDR: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
  ...
  @mcp.tool()
  async def cb_enterprise_edr_ping(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
  ...
  @mcp.tool()
  async def cb_enterprise_edr_get_events_associated_with_process_by_process_guid(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
      process_guid: Annotated[
          str, Field(..., description="Specify a process guid to search events for.")
      ],
      query: Annotated[
          str,
          Field(
              ...
              description=(
                  "Query to execute in process search.For example,"
                  " \u201cnetconn_action:ACTION_CONNECTION_CREATE OR"
                  " netconn_action:ACTION_CONNECTION_ESTABLISHED\u201d"
              ),
          ),
      ],
      record_limit: Annotated[
      ...
      search_criteria: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Specify a search criteria for the request. Currently, only"
                  " \u201cevent_type\u201d values are accepted as the search criteria,"
                  " for example, netconn. Multiple values are accepted as a comma"
                  " separated string."
              ),
          ),
      ],
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          actionProvider="Scripts",  # Assuming constant based on example
          actionName=(
              "CBEnterpriseEDR_Get Events Associated With Process by Process Guid"
          ),
          properties={
              "IntegrationInstance": instance_identifier,
      ...
      try:
        ...
        # Log error appropriately
        print(
            "Error executing action CBEnterpriseEDR_Get Events Associated With Process"
            f" by Process Guid for CBEnterpriseEDR: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
      print(f"Warning: No active integration instance found for CBEnterpriseEDR")
