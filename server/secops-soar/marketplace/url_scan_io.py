...
def register_tools(mcp: FastMCP):
  ...
  @mcp.tool()
  async def url_scan_io_search_for_scans(
      ...
      max_scans: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Number of scans to return per entity. Default: 100, Max: 10000"
  ...
  @mcp.tool()
  async def url_scan_io_url_check(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
      visibility: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Scans on urlscan.io have one of three visibility levels, make sure"
                  " to use the appropriate level for your submission."
              ),
          ),
      ],
      threshold: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Mark entity as suspicious if the score of verdicts is equal or above"
                  " the given threshold. Default is 0, in this case, we consider every"
                  " scanned url as suspicious."
              ),
          ),
      ],
      ...
      add_screenshot_to_insight: Annotated[
          Optional[bool],
          Field(
              default=None,
              description=(
                  "If enabled, action will add a screenshot of the website to the"
                  " insight, if it\u2019s available."
              ),
          ),
      ],
      target_entities: Annotated[
  ...
  @mcp.tool()
  async def url_scan_io_get_scan_full_details(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              "IntegrationInstance": instance_identifier,
              "ScriptName": (
                  "UrlScanIo_Get Scan Full Details"
      ...
      try:
        ...
        # Log error appropriately
        print(
            f"Error executing action UrlScanIo_Get Scan Full Details for UrlScanIo: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
      print(f"Warning: No active integration instance found for UrlScanIo")
