...
def register_tools(mcp: FastMCP):
  ...
  @mcp.tool()
  async def fire_eye_nx_add_ips_policy_exception(
      ...
      victim_ip_subnet: Annotated[
          str,
          Field(
              ...,
              description=(
                  "Specify the IP subnet of the victim that should be used to create a"
                  " new policy exception. Format: x.x.x.x/xx Example: 10.0.0.1/24"
              ),
          ),
      ],
      interface: Annotated[
      ...
      mode: Annotated[
          ...
          Field(
              ...,
              description=(
                  "Specify the mode that should be used in the policy exception."
              ),
          ),
      ],
      name: Annotated[
          ...
          Field(
              default=None,
              description=(
                  "Specify the name for the policy exception. If nothing is specified,"
                  " action will add policy exception with name"
                  " Siemplify_{Interface}_{Mode}"
              ),
          ),
      ],
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              ...
              "ScriptName": (
                  "FireEyeNX_Add IPS Policy Exception"
      ...
      try:
        ...
        print(
            "Error executing action FireEyeNX_Add IPS Policy Exception for"
            f" FireEyeNX: {e}"
        )
  ...
  @mcp.tool()
  async def fire_eye_nx_download_alert_artifacts(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
      ...
      download_path: Annotated[
          str, Field(..., description="Specify where the action should save the files.")
      ],
      target_entities: Annotated[
          List[TargetEntity],
          Field(
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              "IntegrationInstance": instance_identifier,
              "ScriptName": (
                  "FireEyeNX_Download Alert Artifacts"
              ),  # Assuming same as actionName
              "ScriptParametersEntityFields": json.dumps(script_params),
          },
      )
      ...
      try:
        ...
        # Log error appropriately
        print(
            "Error executing action FireEyeNX_Download Alert Artifacts for"
            f" FireEyeNX: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
