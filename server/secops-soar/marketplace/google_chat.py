...
def register_tools(mcp: FastMCP):
  ...
  @mcp.tool()
  async def google_chat_send_advanced_message(
      case_id: Annotated[str, Field(..., description="The ID of the case.")],
      alert_group_identifiers: Annotated[
          List[str], Field(..., description="Identifiers for the alert groups.")
      ],
      space_name: Annotated[
          ...
          Field(
              ...,
              description=(
                  "Specify a space name to send message to. Example space name:"
                  " AAAAdaTsel0"
              ),
          ),
      ],
      message_json_payload: Annotated[
          Union[str, dict],
          Field(
              ...,
              description=(
                  "Specify a JSON payload to send with message. See article"
                  " https://developers.google.com/chat/api/guides/message-formats/cards"
                  " for the examples of messages payload."
              ),
          ),
      ],
      target_entities: Annotated[
    ...
    if instances:
      ...
      action_data = ApiManualActionDataModel(
          ...
          properties={
              "IntegrationInstance": instance_identifier,
              "ScriptName": (
                  "GoogleChat_Send Advanced Message"
      ...
      try:
        ...
        # Log error appropriately
        print(
            "Error executing action GoogleChat_Send Advanced Message for"
            f" GoogleChat: {e}"
        )
        return {"Status": "Failed", "Message": f"Error executing action: {e}"}
    else:
  ...
  async def google_chat_list_spaces(
      ...
      include_user_memberships: Annotated[
          Optional[bool],
          Field(
              default=None,
              description=(
                  "If enabled, user memberships information will be added to the action"
                  " Case Wall table and JSON result."
              ),
          ),
      ],
      target_entities: Annotated[
  ...
  async def google_chat_send_message(
      ...
      message_text: Annotated[
          str, Field(..., description="Specify a message text to send.")
      ],
      target_entities: Annotated[
          List[TargetEntity],
          Field(
