...
async def top_vulnerability_findings(
  ...
  parent = (  # Search across all sources in the project
      f"projects/{project_id}/sources/-"
  )
  # Filter for active, high/critical vulnerability findings
  filter_str = (
      'state="ACTIVE" AND findingClass="VULNERABILITY" AND (severity="HIGH" OR'
      ' severity="CRITICAL")'
  )
  ...
  logger.debug(
      f"Using parent: {parent}, filter: {filter_str}, fetching up to"
      f" {fetch_page_size} findings for sorting."
  )
  ...
  try:
    ...
    if page:
      for item in page.list_findings_results:
        ...
        attack_exposure_score = finding_dict.get(
            "attackExposureScore"
        )  # Directly under finding
    ...
    final_max_findings = (
        max_findings if max_findings is not None and max_findings > 0 else 20
    )
    ...
    more_findings_exist = (
        bool(response_pager.next_page_token)
        or len(all_fetched_findings) == fetch_page_size
    )
    ...
    return {
        "error": "Not Found",
        "details": (
            f"Could not find project '{project_id}' or relevant resources. {str(e)}"
        ),
    }
    ...
    logger.error(
        f"Invalid argument (check filter syntax?) for top findings on {parent}: {e}"
    )
    return {"error": "Invalid Argument", "details": str(e)}
  except Exception as e:
    logger.error(
        f"An unexpected error occurred getting top findings: {e}", exc_info=True
    )
...
async def get_finding_remediation(
  ...
  if not resource_name and not category and not finding_id:
    return {
        ...
        "details": "Either resource_name and category or finding_id must be provided.",
    }
  ...
  try:
    if finding_id:
      ...
      filter_str = (
          f'state="ACTIVE" AND resourceName="{resource_name}" AND category="{category}"'
      )
      logger.info(
          f"Attempting to list active findings for resource: {resource_name}, category:"
          f" {category}"
      )
      scc_request_args = {
          "parent": parent,
    ...
    logger.debug(
        f"Executing list_findings with parent='{parent}', filter='{filter_str}',"
        f" page_size={scc_request_args['page_size']}"
    )
    ...
    if len(results) >= 1:
      if (
          len(results) > 1 and resource_name
      ):  # Warning only needed for resource/category search
        logger.warning(
            f"Multiple ({len(results)}) ACTIVE findings found for resource"
            f" '{resource_name}' and category '{category}'. Using the first one."
        )
    ...
    if first_finding_result:
      ...
      remediation_steps = finding_dict.get(
          "nextSteps", "No remediation steps provided for this finding."
      )
      ...
      if resource_name_from_finding:
        try:
          ...
          logger.debug(
              f"Attempting CAI search with request: {{scope='{cai_scope}',"
              f" query='{cai_request.query}', page_size=1}}"
          )
          ...
          if asset_result:
            ...
            logger.info(
                f"Successfully fetched CAI details for {resource_name_from_finding}"
            )
          else:
            logger.warning(
                "Could not find asset details in CAI for resource:"
                f" {resource_name_from_finding} within scope {cai_scope}"
            )
            asset_details = {
                "error": "Resource details not found in CAI.",
                "resource_name": resource_name_from_finding,
            }
        except google_exceptions.PermissionDenied as cai_e:
          logger.error(
              "Permission denied fetching CAI details for"
              f" {resource_name_from_finding}: {cai_e}"
          )
          asset_details = {
              "error": "Permission Denied fetching resource details from CAI.",
              "details": str(cai_e),
          }
        except google_exceptions.InvalidArgument as cai_e:
          logger.error(
              f"Invalid argument fetching CAI details for {resource_name_from_finding}:"
              f" {cai_e}"
          )
          asset_details = {
              "error": "Invalid Argument fetching resource details from CAI.",
              "details": str(cai_e),
          }
        except Exception as cai_e:
          logger.error(
              "An unexpected error occurred fetching CAI details for"
              f" {resource_name_from_finding}: {cai_e}",
              exc_info=True,
          )
          asset_details = {
              "error": (
                  "An unexpected error occurred fetching resource details from CAI."
              ),
              "details": str(cai_e),
          }
      else:
        logger.warning(
            f"Finding {finding_name} does not have a resourceName, cannot fetch CAI"
            " details."
        )
        asset_details = {
            "warning": "Finding does not have an associated resource name."
        }
      ...
      search_criteria = (
          f"finding ID '{finding_id}' (using name filter)"
          if finding_id
          else (
              f"active finding for resource '{resource_name}' and category '{category}'"
          )
      )
      logger.warning(
          f"No finding found matching {search_criteria} in project '{project_id}'."
          f" Filter: {filter_str}"
      )
      return {
          "message": (
              f"No finding found matching the specified criteria ({search_criteria})."
          )
      }
  ...
  # --- Outer Exception Handling ---
    ...
    logger.error(
        f"Resource not found during SCC operation for project {project_id} (finding_id:"
        f" {finding_id}, resource: {resource_name}): {e}"
    )
    return {
        "error": "Not Found",
        "details": (
            f"Could not find project '{project_id}' or related SCC resources. {str(e)}"
        ),
    }
  except google_exceptions.PermissionDenied as e:
    logger.error(
        f"Permission denied during SCC operation for project {project_id} (finding_id:"
        f" {finding_id}, resource: {resource_name}): {e}"
    )
    return {"error": "Permission Denied", "details": str(e)}
  except google_exceptions.InvalidArgument as e:
    logger.error(
        f"Invalid argument during SCC operation for project {project_id} (finding_id:"
        f" {finding_id}, resource: {resource_name}): {e}"
    )
    return {
        "error": "Invalid Argument",
        "details": f"Check SCC filter syntax or input parameters. {str(e)}",
    }
  except Exception as e:
    # General fallback, including potential CAI client errors not caught inside
    logger.error(
        f"An unexpected error occurred in get_finding_remediation: {e}", exc_info=True
    )
    return {"error": "An unexpected error occurred", "details": str(e)}
...
def main() -> None:
  ...
  if not scc_client:
    logger.critical(
        "SCC Client failed to initialize. MCP server cannot serve SCC tools."
    )
